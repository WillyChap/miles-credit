...
C_BROKEN = "#D55E00"   # exploitable gradient path
...
def _common(ax, with_corrector, del_text, del_fc, del_ec):
    ...
    rawtxt = r"$\hat{y}_t$" if not with_corrector else r"$\hat{y}_{\mathrm{raw}}$"
...
def panel_a(ax):
    ...
    _, net, raw, _, del_ = _common(ax, False, "Delivered output  ($=\\hat{y}_t$)\nbudget NOT closed", FC_OPEN, "0.5")
    loss = box(ax, X_RAW, YL, 2.4, 0.66, "Supervised loss vs $y_t$", FC_LOSS)
...
def panel_c(ax):
    ...
    _, net, raw, cor, del_ = _common(ax, True, "Delivered output\nbudget closed to\nmachine precision", FC_CLOSED, C_TRUTH)
    loss = box(ax, 5.5, YL, 2.5, 0.66, "Supervised loss vs $y_t$\n(sees $\\hat{y}_{\\mathrm{raw}}$)", FC_LOSS)
