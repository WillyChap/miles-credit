...
class Trainer(BaseTrainer):
    def __init__(self, model: torch.nn.Module, rank: int, conf: dict):
        ...
        # Loss-placement flag, decoupled from the penalty weight (enables the 2x2 ablation:
        # supervision on raw vs corrected output x penalty on/off). None = legacy behavior,
        # i.e. supervise the pre-correction prediction iff the penalty weight > 0.
        self.supervise_precorrection = self.conf.get("trainer", {}).get("supervise_precorrection", None)
            ...
            while not stop_forecast:
                ...
                if forecast_step in self.backprop_on_timestep:
                    ...
                    with torch.autocast(enabled=self.amp, device_type="cuda"):
                        # Supervision placement and the conservation penalty are independent.
                        # Placement: supervise the pre-WaterFixer prediction (gradients point
                        # at raw PRECT, no feedback path) vs the corrected output. Penalty:
                        # added whenever the weight > 0, regardless of placement.
                        have_pre = self.flag_water_conserve and (y_pred_pre_water is not None)
                        if self.supervise_precorrection is None:
                            supervise_pre = have_pre and self.water_conservation_loss_weight > 0
                        else:
                            supervise_pre = have_pre and bool(self.supervise_precorrection)
                        y_for_loss = y_pred_pre_water if supervise_pre else y_pred
                        loss = criterion(y.to(y_for_loss.dtype), y_for_loss).mean()
                        if (self.flag_water_conserve and self.water_conservation_loss_weight > 0
